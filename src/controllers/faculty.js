...
const addFacultyToCourse = async (req, res) => {
    ...
    if (course.faculties.includes(faculty._id)) {
        return res.status(409).json({ message: 'Faculty already added to the course' });
    }
...
const removeFacultyFromCourse = async (req, res) => {
    ...
    course.faculties = course.faculties.filter((faculty) => faculty._id != facultyId);
    await course.save();
    faculty.courses_tutoring = faculty.courses_tutoring.filter((course) => course._id != courseId);
    await faculty.save();
    res.status(200).json({ course });
}
